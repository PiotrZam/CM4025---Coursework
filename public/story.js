...
// This should be moved to another js file - will be called from a different page
function getSingleStory(storyID) {

    if(!storyID)
    {
        console.error("Story ID needs to be provided");
        return;   
    }

    var urlString = `/getSingleStory?storyID=${storyID}`
    ...
    $.ajax({
        ...
        error: function () {
            console.error("Error fetching post from the server.");
        }
    });
}
